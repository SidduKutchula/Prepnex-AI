...
/**
 * Renders the Modern Professional Resume template.
 * Features a modern header accent banner, sleek typography (helvetica),
 * and clean section pill markers.
 *
 * @param {Object} resumeData 
 * @param {jsPDF} [pdfDoc] 
 * @returns {jsPDF}
 */
export function renderModernTemplate(resumeData, pdfDoc = null) {
    ...
    const drawSectionHeader = (title) => {
        ...
        // Draw colored accent box indicator
        doc.setFillColor(37, 99, 235); // Royal Blue accent
        doc.rect(margin, y - 2, 4, headingSize + 2, 'F');

        doc.setFont('helvetica', 'bold');
        doc.setFontSize(headingSize);
        doc.setTextColor(30, 41, 59);
        doc.text(title.toUpperCase(), margin + 10, y + headingSize - 2);
    ...
    doc.setTextColor(37, 99, 235); // Accent color name
    ...
    if (resumeData.title) {
        ...
        doc.text(resumeData.title.toUpperCase(), margin, y);
        y += (fontSize + 1) * 1.3;
    }
    ...
    if (resumeData.projects && resumeData.projects.length > 0) {
        ...
        resumeData.projects.forEach(proj => {
            ...
            if (proj.tech) {
                ...
                doc.setFontSize(fontSize - 0.5);
                doc.setTextColor(100, 116, 139);
                doc.text(`Tech: ${proj.tech}`, margin + contentWidth, y, { align: 'right' });
            }
    ...
    if (resumeData.achievements && resumeData.achievements.length > 0) {
        drawSectionHeader('Key Achievements & Awards');
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(fontSize);
        doc.setTextColor(51, 65, 85);
    ...
    // Metadata
    doc.setProperties({
        title: `${resumeData.name || 'Candidate'} - Modern Resume`,
...
export default renderModernTemplate;
